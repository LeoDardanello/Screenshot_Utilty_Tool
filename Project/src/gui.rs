...
pub fn gui_mode3(my_app: &mut MyApp, frame: &mut eframe::Frame, ui: &mut egui::Ui) {
    if my_app.edit_image.screens.len() > 0 {
        screenshot::visualize_image(&mut my_app.edit_image, ui, frame.info().window_info.size);
    } else {
...
pub fn gui_mode5(my_app: &mut MyApp, frame: &mut eframe::Frame, ui: &mut egui::Ui) {
    ...
    let mut limits = (
        ...
        80.0,
        info.size[0] - 10.0,
        ((info.size[0] - 20.0) * my_app.image[my_app.n_monitor].size.1 as f32)
            / my_app.image[my_app.n_monitor].size.0 as f32,
    );
...
pub fn gui_mode6(my_app: &mut MyApp, frame: &mut eframe::Frame, ui: &mut egui::Ui) {
    ...
    let my_rect = egui::Rect::from_two_pos(
        egui::pos2(limits.0, limits.1),
        egui::pos2(limits.2, limits.3),
    );
                    ...
                    my_app.paint.retain(|x| {
                        if x.start.is_some() && x.end.is_some() {
                            if x.draw == Paints::Circle {
                                ...
                                if pos.x as usize >= (c.x - r) as usize
                                    && pos.x as usize <= (c.x + r) as usize
                                    && pos.y as usize >= (c.y - r) as usize
                                    && pos.y as usize <= (c.y + r) as usize
                                {
                                    return false;
                                } else {
                                ...
                                let p2 = x.end.unwrap();
                                if pos.x as usize >= cmp::min(p1.x as usize, p2.x as usize)
                                    && pos.x as usize <= cmp::max(p1.x as usize, p2.x as usize)
                                    && pos.y as usize >= cmp::min(p1.y as usize, p2.y as usize)
                                    && pos.y as usize <= cmp::max(p1.y as usize, p2.y as usize)
                                {
                                    return false;
                                } else {
                                    return true;
    ...
    for figure in my_app.paint.iter_mut() {
        if figure.start.is_some() && figure.end.is_some() {
            if figure.draw == Paints::Arrow {
                ...
                painter.circle(
                    figure.start.unwrap(),
                    figure.start.unwrap().distance(figure.end.unwrap()),
                    egui::Color32::TRANSPARENT,
                    egui::Stroke {
                        width: 1.5,
                ...
                let stroke = egui::Stroke::new(
                    ...
                    egui::Color32::from_rgba_unmultiplied(
                        figure.color.unwrap().r(),
                        figure.color.unwrap().g(),
                        figure.color.unwrap().b(),
                        50,
                    ),
                );
